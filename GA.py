...
price_col = find_column(["price","ticket"])
demand_col = find_column(["person","sold","demand","quantity","attendance","number"])
...
objective_type = st.sidebar.selectbox("Optimization Objective", ["Single Objective", "Multi Objective"])
w1, w2 = 1.0, 0.0
...
if st.button("Run Genetic Algorithm"):
    ...
    for gen in range(GENERATIONS):
        ...
        # Elitism
        population_sorted = [p for _, p in sorted(zip(fitness_values, population), reverse=True)]
    ...
    st.line_chart(pd.DataFrame({"Best Fitness": best_fitness_history, "Average Fitness": avg_fitness_history}))
